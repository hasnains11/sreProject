...
class ExternalUnitMulticlientTest : public testing::Test {
 protected:
 public:
...
class TestClient : public chaos::common::external_unit::ExternalUnitClientEndpoint {
...
TestClient::TestClient(int _client_id)
    : ExternalUnitClientEndpoint("Client"), client_id(_client_id) {
  echo_received               = false;
...
void TestClient::handleNewConnection(const std::string& connection_identifier) {
  current_connection = connection_identifier;
  connection_event_counter++;
}
void TestClient::handleDisconnection(const std::string& connection_identifier) {
